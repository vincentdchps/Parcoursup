...
## ≠ Différence entre les deux phases 

On observe que la phase 1 présente une modélisation sous la forme d'une seule et unique table qui contient l'entièreté des données de la base. Cette modélisation entraine d'importantes redondances car les informations relatives aux départements, aux établissements et aux formations sont dupliquées à chaque nouvelle ligne de statistiques. Cela provoque des risques d'incohérence lors des mises à jour et alourdit la base. La phase 2 propose un schéma relationnel normalisé décomposé en quatre tables distinctes liées par des clés étrangères : departement, etablissement, formation et statistiques. Cette modélisation résout les problèmes de la phase 1. Cette phase permet donc tout d’abord une élimination des redondances. En effet,  la séparation des entités permet de stocker les informations descriptives une seule fois. Le libellé d'un département ou le nom d'un établissement n'est plus répété sur chaque ligne de statistiques. Elle permet également une correction des anomalies de modification : si le contrat d'un établissement change, la modification se fait sur une seule ligne dans la table etablissement, ce qui garantit la cohérence de la base et évite les erreurs de mise à jour partielles. De plus elle met aussi en place une correction des anomalies d'insertion et de suppression puisque le nouveau schéma permet d'insérer un département ou un établissement même si aucune formation ou statistique ne lui est encore attribué . Inversement , la suppression d'une ligne de statistiques n'efface plus les données de la formation ou de l'établissement lié. Et enfin celle-ci permet une garantie de l'intégrité car la mise en place des clés étrangères empêche l'ajout de statistiques pour une formation inexistante, permettant d’avoir une base de donnée cohérente et fiable pour les requêtes.
